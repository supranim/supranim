...
proc generateHeaders(requestUrl: Uri, httpMethod: HttpMethod, headers: HttpHeaders,
  ...
  if not headers.hasKey("Host"):
    if requestUrl.port == "":
      if requestUrl.scheme == "unix":
        add(result, "Host: UDS (Unix Domain Sockets)" & httpNewLine)
      else:
...
type
  ...
  UnixHttpClient* = HttpClientBase[Socket]
...
proc newUnixHttpClient*(unixPath: string, userAgent = defUserAgent, maxRedirects = 5,
                    sslContext = getDefaultSSL(),
                    timeout = -1, headers = newHttpHeaders()): UnixHttpClient =
  ## Creates a new UnixHttpClient instance
  ##
  ## `userAgent` specifies the user agent that will be used when making
  ## requests.
  ##
  ...
  result.userAgent = userAgent
  result.maxRedirects = maxRedirects
  result.proxy = newProxy(unixPath)
  result.timeout = timeout
  result.onProgressChanged = nil
  result.bodyStream = newStringStream()
  result.getBody = true
...
proc newConnection(client: HttpClient | AsyncHttpClient,
  ...
  if client.currentURL.hostname != url.hostname or
    ...
    when client is HttpClient:
      ...
      elif getSockDomain(client.socket.getFd) == AF_UNIX:
        client.socket.connectUnix(client.proxy.url.path)
        client.proxy.url.hostname = "UDS (Unix Domain Sockets)"
...
proc downloadFile*(client: HttpClient | UnixHttpClient, url: Uri | string, filename: string) =
...
when isMainModule:
 # var client = newUnixHttpClient("unix:///tmp/supranim_starter.socket")
 # let res = client.get("unix:///")
 # echo res.bodyStream.readAll()

 var client = newHttpClient()
 let res = client.get("http://127.0.0.1:8000")
 echo res.bodyStream.readAll()