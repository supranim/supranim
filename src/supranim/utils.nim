...
proc newImport*(idents: varargs[string]): NimNode =
  result = newNimNode(nnkImportStmt)
  for i in idents:
    result.add ident(i)

proc newImport*(idents: seq[string]): NimNode =
  result = newNimNode(nnkImportStmt)
  for i in idents:
    result.add ident(i)
...
proc newExport*(idents: seq[string], comment = ""): NimNode =
  result = newStmtList()
  if comment.len != 0:
    result.add(newCommentStmtNode(comment))
  var exportModules = newNimNode(nnkExportStmt)
  for i in idents:
    exportModules.add ident(i)
  result.add exportModules
