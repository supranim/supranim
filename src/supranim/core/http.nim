...
when defined supraServerMummy:
  ...
  export Request
  # proc initSettings*(port: Port, bindAddr = "", domain: Domain, numThreads: int, startup: proc()): Settings =
  #   discard

  proc runServer*(onRequest: proc(req: Request) {.gcsafe.}) =
    ## Starts the HTTP server using `pkg/mummy`
    ## and runs the request handler.
    var server = newServer(onRequest, workerThreads = 100)
    server.serve(port = Port(8080))
  ...
  proc send*(req: Request, code: HttpCode, body: sink string) =
    req.respond(code.int, body = body)
...
elif defined supraServerAsyncHttp:
  import std/asynchttpserver
  proc runServer*(onRequest: OnRequest, settings: Settings) =
    discard
else:
  include ./http/httpbeast/httpbeast

  proc runServer*(onRequest: OnRequest, settings: Settings) =
    ## Starts the HTTP server using `pkg/httpbeast`
