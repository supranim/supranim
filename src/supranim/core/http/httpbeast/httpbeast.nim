import selectors, net, nativesockets, os, httpcore,
        asyncdispatch, strutils, posix
import parseutils
...
proc eventLoop(
  ...
  if settings.domain != Domain.AF_UNIX:
    ...
    server.bindUnix(settings.bindAddr)
  # Disable Nagle Algorithm if the server socket
  # is likely to be a TCP socket.
  ...
  while true:
    ...
    processEvents(selector, events, ret, onRequest, settings.domain)
